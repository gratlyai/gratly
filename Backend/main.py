...
def _fetch_restaurant_key(user_id: int) -> Optional[int]:
    ...
    try:
        cursor.execute(
            "SELECT restaurantkey FROM calctip.userRestaurant WHERE userid = %s LIMIT 1",
            (user_id,),
        )
        row = cursor.fetchone()
        return row["restaurantkey"] if row else None
    finally:
        cursor.close()
...
def _fetch_restaurant_name(user_id: int) -> Optional[str]:
    ...
    try:
        cursor.execute(
            """
            SELECT rd.name
            FROM calctip.userRestaurant ur
            JOIN calctip.onboarding ob ON ur.restaurantkey = ob.restaurantkey
            JOIN calctip.RestaurantDetails rd ON rd.restaurantID = ob.restaurantID
            WHERE ur.userid = %s
            LIMIT 1
            """,
            (user_id,),
...
try:
    ...
    # Create users table if it doesn't exist (assuming 'id' is AUTO_INCREMENT PRIMARY KEY)
    cursor.execute("""
...
def signup(data: dict):
    ...
    cursor.execute(
        """
        INSERT INTO users (firstname, lastname, email, phonenumber, password_hash)
        VALUES (%s, %s, %s, %s, %s)
        """,
...
def login(data: dict):
    ...
    if verify_password(password, user["password_hash"]):
        restaurant_key = _fetch_restaurant_key(user["id"])
        restaurant_name = _fetch_restaurant_name(user["id"])
        return {
            "success": True,
            "user_id": user["id"],
