...
def _get_env_or_ini(config, section, key, env_key=None, default=None):
...
def _normalize_days_back(value: Optional[int]) -> Optional[int]:
   if value is None:
       return None
   try:
       parsed = int(value)
...
def _resolve_date_range(days_back: Optional[int]):
   ...
   return start_date, end_date, business_dates


def main(days_back: Optional[int] = None):
   script_dir = os.path.dirname(os.path.abspath(__file__))
   config_path = os.path.join(script_dir, 'setting.ini')
   config = load_config(config_path)
   log(f"Loaded config from {config_path}")
  
   resolved_days_back = _normalize_days_back(days_back)
   start_date, end_date, business_dates = _resolve_date_range(resolved_days_back)
   log(
       f"Using date range {start_date} to {end_date} "
       f"(business dates {business_dates[0]} to {business_dates[-1]})"
   )
  
   # Database connection details
   db_config = {
       'host': _get_env_or_ini(config, 'DATABASE', 'host', 'DB_HOST'),
       'user': _get_env_or_ini(config, 'DATABASE', 'user', 'DB_USER'),
       'password': _get_env_or_ini(config, 'DATABASE', 'password', 'DB_PASSWORD'),
       'database': _get_env_or_ini(config, 'DATABASE', 'database', 'DB_NAME'),
       'port': int(_get_env_or_ini(config, 'DATABASE', 'port', 'DB_PORT', '3306')),
   }
  
   # Get all the URLs from the setting.ini
  
   urls = _get_env_or_ini(config, 'URLS', 'url', 'TOAST_URLS')
   delimiter = ','
           ...
           if url == 'https://ws-api.toasttab.com/orders/v2/ordersBulk':
               for business_date in business_dates:
                   ...
                   try:
                       ...
                       conn.commit()
                       log(f"Inserted {len(all_orders_sql_data)} orders for {headers_init} ({business_date})")
                   except Exception as e:
                       log(f"Insert orders failed for {headers_init} ({business_date}): {e}")
                       conn.rollback()
