...
IS_ACTIVE BOOLEAN DEFAULT TRUE,      -- 1 - ACTIVE, 0 - ARCHIVED
CREATEDDATE DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- Add PREPAYOUT_DEDUCTION and PAYOUT_FEE columns to PAYOUT_APPROVAL_ITEMS
SET @col_exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='GRATLYDB' AND TABLE_NAME='PAYOUT_APPROVAL_ITEMS' AND COLUMN_NAME='PREPAYOUT_DEDUCTION');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE GRATLYDB.PAYOUT_APPROVAL_ITEMS ADD COLUMN PREPAYOUT_DEDUCTION DECIMAL(12,2)', 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @col_exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='GRATLYDB' AND TABLE_NAME='PAYOUT_APPROVAL_ITEMS' AND COLUMN_NAME='PAYOUT_FEE');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE GRATLYDB.PAYOUT_APPROVAL_ITEMS ADD COLUMN PAYOUT_FEE DECIMAL(12,2)', 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- Ensure IS_ACTIVE column exists on PAYOUT_SCHEDULE for archive functionality
SET @col_exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='GRATLYDB' AND TABLE_NAME='PAYOUT_SCHEDULE' AND COLUMN_NAME='IS_ACTIVE');
SET @sql := IF(@col_exists = 0, 'ALTER TABLE GRATLYDB.PAYOUT_SCHEDULE ADD COLUMN IS_ACTIVE BOOLEAN DEFAULT TRUE', 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Audit table for tracking approval edits and changes for reconciliation
CREATE TABLE IF NOT EXISTS GRATLYDB.PAYOUT_APPROVAL_HISTORY (
  HISTORY_ID INT AUTO_INCREMENT PRIMARY KEY,
  PAYOUT_APPROVALID INT,
  PAYOUT_APPROVAL_ITEMID INT,
  RESTAURANTID INT NOT NULL,
  PAYOUT_SCHEDULEID INT NOT NULL,
  ...
  EMPLOYEE_NAME VARCHAR(128),
  JOBTITLE VARCHAR(128),
  FIELD_NAME VARCHAR(64),               -- Field that was changed (PAYOUT_PERCENTAGE, NET_PAYOUT, etc.)
  OLD_VALUE VARCHAR(256),               -- Previous value before edit
  NEW_VALUE VARCHAR(256),               -- New value after edit
  CHANGED_BY_USERID INT NOT NULL,       -- User who made the change
  CHANGE_TYPE VARCHAR(32),              -- EDIT, ADD_RECEIVER, REMOVE_RECEIVER, INITIAL_SNAPSHOT
  CREATED_AT DATETIME DEFAULT CURRENT_TIMESTAMP,
  INDEX IDX_HISTORY_APPROVAL (PAYOUT_APPROVALID),
  INDEX IDX_HISTORY_SCHEDULE (PAYOUT_SCHEDULEID, BUSINESSDATE),
  INDEX IDX_HISTORY_USER (CHANGED_BY_USERID),
  INDEX IDX_HISTORY_EMPLOYEE (EMPLOYEEGUID)
);

-- AI Agent conversation tables for chatbot functionality
CREATE TABLE IF NOT EXISTS GRATLYDB.AGENT_CONVERSATIONS (
  ...
  INDEX IDX_AGENT_CONV_RESTAURANT (RESTAURANT_ID)
);

CREATE TABLE IF NOT EXISTS GRATLYDB.AGENT_MESSAGES (
